...
export function listPlans(page = 1, pageSize = 20): Promise<{ plans: Plan[]; page: PageInfo }> {
  return apiRequest(`/plans?page=${page}&page_size=${pageSize}`);
}
...
export function listWorkoutDays(planId: string): Promise<{ workout_days: WorkoutDay[]; page: PageInfo }> {
